...
import {updateUser,checkUserExist} from "../../../../api/api";
...
const Page = () => {
    ...
    const handleSubmit = async () => {
        ...
        if(response.statusCode === 404){
            ...
            emailjs.send('service_tmnbpb6', 'template_nbh6d85', emailData)
                .then(response => {
                    setIsSuccess(true);
                    setShow(true)
            ...
            <div className="bg-white p-8 rounded shadow-md w-96">
                {
                    changePassword ?
                        <>
                            <h2 className={`text-2xl font-semibold ${letChangePassword?"mb-2":"mb-6"}`}>Reset Password</h2>
                            {
                                letChangePassword && <span className={'text-red-500 text-sm'}>Password and confirm password does not match</span>
                            }
                            <div className="my-4">
                        ...
                        <>
                            ...
                            {isSuccess ? (
                                <>
                                    {error ? "" : <div className="text-green-600 mb-4">ResetCode sent to your email address</div>}
                                    {show &&
                                        <div className="mb-4">
                                            <input
                                                type="text"
                                                id="resetCode"
                                                className={`mt-1 ${error? "mb-1" : "mb-4"} p-2 w-full border rounded-md`}
                                                onChange={(e) => setResetCode(e.target.value)}
                                                required
                                            />
                                            {error && <span className={'text-red-500 text-sm flex flex-col mb-3'}>Your ResetCode is Expired!!</span>}
                                            <button
                                                type="button"
